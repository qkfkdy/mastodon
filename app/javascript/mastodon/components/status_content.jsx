...
import { OcSceneUI } from './oc_scene';
...
class StatusContent extends PureComponent {
  ...
  handleElement = (element, { key, ...props }, children) => {
    if (element instanceof HTMLAnchorElement) {
      ...
      return null;
    } else if (element.classList.contains('e-oc-scene')) {
      try {
        const metadata = JSON.parse(element.textContent);
        return (
          <OcSceneUI
            key={key}
            ...
            botAcct={this.props.status.getIn(['account', 'acct'])}
          />
        );
      } catch (e) {
        return null;
